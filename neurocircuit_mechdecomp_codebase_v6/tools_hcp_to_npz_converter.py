"""
HCP-YA Converter: CIFTI → Cortical Parcels + Striatum Supervoxels → NPZ
-----------------------------------------------------------------------
...
clusters the **striatum** into **K supervoxels (S)**, and writes a per-run **.npz**
with arrays shaped `(T, P_group)` ready for training:
...
Optionally, if you provide masks and Ks, it can also cluster:
  • 'GPi', 'GPe', 'STN', 'Th', 'MB'  → added to the NPZ if supplied.
...
Examples
--------
# Process the four HCP-YA resting runs for one subject (Schaefer400; K=1000 S voxels)
...
# If you already extracted subcortex to a 4D NIfTI
python hcp_to_npz_converter.py make-npz \
  --dtseries rfMRI_REST1_LR_Atlas_MSMAll.dtseries.nii \
  --dlabel   Glasser360.dlabel.nii \
  --subcortex-nii subcortex_4d.nii.gz \
  --striatum-mask striatum_mask_MNI.nii.gz \
  --K-S 800 \
  --out data/sub-100307_REST1_LR.npz

# Batch a folder of runs into NPZ and write a manifest (one line per NPZ)
python hcp_to_npz_converter.py batch \
  --runs-root /path/to/HCP/sub-*/MNINonLinear/Results \
...
def parcellate_cortex(dtseries_path: str, dlabel_path: str) -> np.ndarray:
    ...
    if have_wb():
        with tempfile.TemporaryDirectory() as td:
            ...
            C = np.loadtxt(txt)
            # wb writes P×T or T×P depending on version; make it (T,P)
            C = C if C.shape[0] > C.shape[1] else C.T
    ...
    # Heuristic: take argmax over label axis to get per-grayordinate labels
...
def build_supervoxels(subcortex_nii: str, mask_nii: str, K: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if mask.shape != X.shape[:3]:
        raise ValueError("Striatum mask shape does not match subcortex volume shape.")
...
# -------------------------------
# Optional: generic nucleus clustering
# -------------------------------
...
def cmd_make_npz(args):
    ...
    if args.subcortex_nii is None:
        ...
        S, S_centers, _ = build_supervoxels(args.subcortex_nii, args.striatum_mask, args.K_S)
        arrays = {'C': C.astype(np.float32), 'S': S.astype(np.float32)}

    # 3) Optional nuclei
    if args.GPi_mask and args.K_GPi>0:
        R = cluster_region(args.subcortex_nii or subvol, args.GPi_mask, args.K_GPi)
        if R is not None: arrays['GPi'] = R.astype(np.float32)
    if args.GPe_mask and args.K_GPe>0:
        R = cluster_region(args.subcortex_nii or subvol, args.GPe_mask, args.K_GPe)
        if R is not None: arrays['GPe'] = R.astype(np.float32)
    if args.STN_mask and args.K_STN>0:
        R = cluster_region(args.subcortex_nii or subvol, args.STN_mask, args.K_STN)
        if R is not None: arrays['STN'] = R.astype(np.float32)
    if args.Th_mask and args.K_Th>0:
        R = cluster_region(args.subcortex_nii or subvol, args.Th_mask, args.K_Th)
        if R is not None: arrays['Th'] = R.astype(np.float32)
    if args.MB_mask and args.K_MB>0:
        R = cluster_region(args.subcortex_nii or subvol, args.MB_mask, args.K_MB)
        if R is not None: arrays['MB'] = R.astype(np.float32)

    # 4) Save NPZ
    write_npz(args.out, arrays)

    # 5) Save coords if requested
    ...
    # Optionally save user-provided ROI centroids CSV as roi_coords.npy (format: label,x,y,z)
...
def cmd_batch(args):
    ...
    for dt in tqdm(paths, desc='batch'):
        subj = None
        # Try to grab subject ID from path (HCP-style '100307' or 'sub-XXXXX')
        ...
        # Prepare subcortex 4D if not provided
        with tempfile.TemporaryDirectory() as td:
...
def main():
    ap = argparse.ArgumentParser(description='HCP-YA CIFTI→NPZ converter (cortex parcels + striatum supervoxels)')
